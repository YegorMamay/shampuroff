...
if (have_comments()) : ?>
    ...
    <ol class="commentlist">
        <?php wp_list_comments('type=comment&callback=bw_comment'); ?>
    </ol>
                    ...
                    <?php if (is_user_logged_in()) : ?>
                        <p>
                            ...
                            <a href="<?php echo wp_logout_url(get_permalink()); ?>"
                               title="<?php __('Log out of this account', 'brainworks'); ?>"><?php echo __('Log out', 'brainworks') . ' <i class="glyphicon glyphicon-arrow-right"></i>'; ?></a>
                        </p>
                    <?php else : ?>

                        <div class="form-group">
                            <label class="screen-reader-text" for="author"><?php _e('Your name', 'brainworks');
                                if ($req) echo ' <span class="text-muted">' . __('(required)', 'brainworks') . '</span>'; ?></label><br>
                            <input type="text" name="author" id="author"
                                   placeholder="<?php _e('Your name', 'brainworks'); ?>"
                                   value="<?php echo esc_attr($comment_author); ?>" <?php if ($req) echo 'aria-required="true"'; ?>>
                        </div>
                        <div class="form-group">
                            <label class="screen-reader-text" for="email"><?php _e('Your email address', 'brainworks');
                                if ($req) echo ' <span class="text-muted">' . __('(required, but will not be published)', 'brainworks') . '</span>'; ?></label><br>
                            <input type="email" name="email" id="email"
                                   placeholder="<?php _e('Your email address', 'brainworks'); ?>"
                                   value="<?php echo esc_attr($comment_author_email); ?>" <?php if ($req) echo 'aria-required="true"'; ?>>
                        </div>
